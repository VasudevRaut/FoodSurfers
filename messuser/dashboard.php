...
session_start();
// echo "Vasduvvvvvvvvvvvvvvvvvvvvv";
...
// Create a connection
// $conn = new mysqli("localhost", include 'db_con.php';"root", "", "foodsurfers");
include 'db_con.php';
// Check connection
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
...
// echo "------------------------------------";
$query = "SELECT `messmenu`.*,`mess`.* FROM    `messmenu`,`mess`,`favorites_mess`  WHERE `messmenu`.Email = `mess`.Email AND  `mess`.Email=`favorites_mess`.owner_email AND `favorites_mess`.user_email = '$email' AND open_closed_status='true' AND date='$currentDate' AND `messmenu`.meal_type = '$mealType' ";
...
?>
            ...
            <div class="card" onclick="handleCardClick('<?php echo $row['Email']; ?>')"  >
                <div class="inBox">
                    <div class="desc">
                    ...
                    <img class="favorite-icon" src="../Images/fav.png" width = "50px" id="fav <?php echo $row['Email']; ?>" height="50px" alt="Favorite" onclick="handleRatingClick('<?php echo $row['Email']; ?>')"> 
                    ...
                    <!-- <label id="myLabel" class="hidden-label"><?php echo $row['Email']; ?></label> -->
...
xhr.send();
        ...
        label.style.visibility = (label.style.visibility === 'visible') ? 'hidden' : 'visible';

        // Get the value using JavaScript
        var dynamicValueFromPHP = label.innerHTML;
        console.log("Dynamic Value from PHP: " + dynamicValueFromPHP);
    }


    var imagePaths = ["../Images/fw.png","../Images/fw.png"];
    // Variable to keep track of the current image index
    var currentImageIndex = 0;
    function handleRatingClick(cardNumber) {
        // var label = document.getElementById("myLabel" + cardNumber);
        ...
        console.log("Card " + cardNumber + " clicked!");
            ...
            // Define the request URL, including any parameters you need to send to the server
            var url = 'removefromFavraite.php?location=' + encodeURIComponent(cardNumber);
